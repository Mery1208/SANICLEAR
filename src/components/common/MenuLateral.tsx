...
const MenuLateral: React.FC<MenuLateralProps> = ({ items }) => {
    return (
        <aside className="w-[260px] bg-[#92cbf8] border-r border-[#7ab9ee] overflow-y-auto shrink-0 flex flex-col py-6 px-4 font-sans">
                ...
                {items.map(item => {
                    if (item.disabled) {
                        return (
                            <div
                                key={item.id}
                                className="flex items-center justify-between w-full py-3 px-5 rounded-3xl cursor-not-allowed transition-all duration-200 text-left text-[15px] font-semibold border shadow-sm bg-slate-100 text-slate-400 border-slate-200 opacity-60"
                            >
                    ...
                    return (
                        <NavLink
                            ...
                            className={({ isActive }) => `
                                flex items-center justify-between w-full py-3 px-5 rounded-3xl cursor-pointer transition-all duration-200 text-left text-[15px] font-semibold border shadow-sm
                                ${isActive 
